...
{\bf R-hints:}
\begin{itemize}
\item A matrix can be created for example like \\
\texttt{A <- matrix(c(4,2,3,0,3,6),byrow=TRUE,nrow=2)} \ .
...
\begin{enumerate}[a)]  \item Repeat the example given in slides 35--38 using R. \\ 
\item Repeat the above procedure 100 times. In each iteration, store the estimated coefficient vector $(\hat\beta_0,\hat\beta_1,\hat\beta_2)$. Note that the estimates should be different in each iteration, because another random vector \texttt{t.e} is used.\\ 
...
\item Define a results matrix of dimension $3\times 100$ that contains in each column the estimates $(\hat\beta_0,\hat\beta_1,\hat\beta_2)$ of an experiment, and fill it with the results of the 100 iterations:\\
