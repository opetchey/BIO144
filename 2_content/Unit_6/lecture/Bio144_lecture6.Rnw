...
L: $\hat{y}_i = \Sexpr{format(worm.coef[1,1],2,2,2)} + \Sexpr{round(worm.coef[2,1],2)}\cdot \text{MAGENUMF}$ \\[2mm]
...
~\\[5mm]
To remember: The \texttt{Gattung} or \texttt{MAGENUMF:Gattung} terms in the model output are always the differences (in intercept or slope) {\bf with respect to the reference level}.
...
~\\[4mm]
Remember lectures 4 and 5 (or Stahel script p.33): We have already used the $F$-test for categorical variables (see $F$-test for the earthworms in lecture 4 or cooking rule on slide 6 in lecture 5). This was equivalent to testing if all $\beta$s that belong to a categorical variable are =0 at the same time. \\[2mm]
...
\frame[containsverbatim]{
The interaction plot indicates that an interaction between SORTE and DUENGER is needed in the analysis. The results and residal plots are given here and on the next slide:
