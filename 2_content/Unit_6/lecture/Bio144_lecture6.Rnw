...
{\bf Example:} Cholesterol levels [mg/ml] for 30 women from three US states (Iowa, Nebraska, Arizona), were measured. \\[2mm]
...
with $\bm{\tilde\beta}$ the vector of regression coefficients and $\bm{e}$ the vector of residuals.
\end{itemize}
}
