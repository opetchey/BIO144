...
\date[]{Lecture 6: ANCOVA, Introduction to Linear Algebra\\ 30./31. March 2017}
...
~\\[2mm]
$\rightarrow$ $p=0.167$ seems not to be significant.
}
...
@
~\\[2mm]
{\small {\bf Note:} the $p$-value for `state' is now the same as on the previous slide (anova table).} Reason: anova tests the models against one another in the {\bf order} specified.
} 
