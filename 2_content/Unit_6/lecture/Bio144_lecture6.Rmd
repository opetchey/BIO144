...
We have already used the $F$-test for categorical variables (see $F$-test for the earthworms in lecture 5). This was equivalent to testing if all $\beta$s that belong to a categorical variable are =0 at the same time. 
...
\ 
$\hat{(\beta\gamma)}_{beef/high}=\hat{(\beta\gamma)}_{beef/low}=\hat(\beta\gamma)_{cereal/low}=0$, $\hat{(\beta\gamma)}_{cereal/high}= -18.8$.
