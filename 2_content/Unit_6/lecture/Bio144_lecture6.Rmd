...
Let's go back to the earthworm example, and fit a model that allows species-specific intercepts and slopes:
...
We have now actually fitted __three__ models, one model for each species:

\colorbox{lightgray}{\begin{minipage}{14cm}  

L: $\hat{y}_i =`r format(worm.coef[1,1],2,2,2)`+
`r format(worm.coef[1,1],2,2,2)` + `r round(worm.coef[2,1],2)` \cdot \text{MAGENUMF}$  
...
* The "*`Gattung`*" terms in the model output are the __differences in intercepts__ with respect to the reference level.

* The "*`MAGENUMF:Gattung`*" terms in the model output are the __differences in slopes__ with respect to the reference level.

## Testing for an interaction term

If we want to find out if the interaction term for a factor covariate with more than two levels is relevant, we again need an $F$-test, that is, use the `anova()` function:
...
Four different hybrid Mais breeds were grown to asses their yield. Each breed was grown at 5 different locations.  

__Questions:__ Are there differences in yield among the four hybrids?

\tiny \alert{Attention:} The question is about \emph{any} difference. More precisely, we could ask if any breed is different from any other.
...
Formulate a model that is able to \alert{test simultaneously} whether there is an __overall difference between the groups.__ That is, ask only __one question!__
...
* $y_{ij}$= "Yield of the $j^\text{th}$ plant of hybrid $i$"
* $\mu_i$="Mean yield of hybrid $i$"
* $\epsilon_{ij}\sim(0,\sigma^2)$ is an independent error term. 

Typically, this is __rewritten as__
\begin{eqnarray*}
y_{ij} = \mu + \beta_i + \epsilon_{ij} \ ,
\end{eqnarray*}
where $\mu + \beta_i = \mu_i$ from above, thus the __group mean__ of group $i$.

## Single factor ANOVA (Einfaktorielle Varianzanalyse)

More generally, this leads us to the \alert{single factor ANOVA:}
...
j &=& 1,\ldots, n_i , \nonumber\\ 
\epsilon_{ij} &\sim& (0,\sigma^2) \quad i.i.d. \nonumber
\end{eqnarray}
\end{minipage}}

* $\mu$ plays the role of the \alert{intercept} $\beta_0$ in standard regression models.
* The estimation of $\mu$, and the $\beta$ coefficients is again done by \alert{least squares minimization}.
* The $\epsilon_{ij} \sim (0,\sigma^2)$  $i.i.d.$ assumption is again crucial, so \alert{model checking} will be needed again.

## 

Attention: Model (1) is overparameterized, thus an additional constraint is needed! Most popular:

* $\beta_1=0$ (alert{treatment contrast}; default in R).

Interpretation: Group 1 is usually chosen such that it is some sort of \alert{reference group} or \alert{reference level}, for example a standard diet, while groups 2, 3, etc. correspond to novel diets whose effect is tested in an experiment.  
...
__Interpretation: The levels of the factor are now the different group memberships.__
...
__Aim of ANOVA__: to test _globally_ if the groups differ. That is:
...
Remember lectures 4 and 5: We have already used the $F$-test for categorical variables (see $F$-test for the earthworms in lecture 4 or cooking rule on slide 6 in lecture 5). This was equivalent to testing if all $\beta$s that belong to a categorical variable are =0 at the same time. 

$\rightarrow$ Equivalent to testing if the categorical covariate is needed in the model.

This is \alert{the very same problem here}, thus we need the $F$-test again!
...
* $MS_G$: Quantifies the variability __between__ groups.
* $MS_E$: Quantifies the variability __within__ groups.

__Example:__

```{r fig.width=4, fig.height=4,out.width="4.7cm", fig.align='center',echo=FALSE, message=FALSE}
qplot(as.factor(d.mais$HYBRID),d.mais$YIELD) + xlab('Hybrid (Sorte') + ylab('Yield (Ertrag)') +
geom_hline(yintercept = 62.555, col = 'red') + geom_point(data= d.agg, aes(x = HYBRID, y = YIELD, col = 'red'), shape = 15)
```

* $MS_G$ captures the variability among the four means (\alert{red squares})
* $MS_E$ captures the variability of the $y_{ij}$ \alert{red line}.
...
An overview of the results is typically given in an ANOVA table (Varianzanalysen-Tabelle):  
...
Model checking is identical to all we did so far, because we are __still working with linear models!__
...
## 
__Always__ when we needed to do an $F$-test and when categorical covariates were involved, the `anova()` table is required: 
...
```
\normalsize
You can see that the value of $F=$ `r format(summary(r.mais)$fstatistic[1],nsmall=2,digits=2)` is $F$-distributed with 3 and 16 degrees of freedom, and the $p$-value of the test ``$\beta_2=\beta_3=\beta_4=0$'' is $<0.0001$.
...
\end{minipage}}
\small 
__Exercise:__ Look at the table a bit closer. How are `Df`, `Sum Sq`, `Mean Sq`, `F value` and `Pr(<F)} related?`
...
\normalsize
The table contains the estimates of the intercept `r format(summary(r.mais)$coef[1,1],2,2,2)` ($\mu$ in ANOVA notation, $\beta_0$ in regression notation), and estimates for $\beta_2$, $\beta_3$, $\beta_4$ (while the reference was set to $\beta_1=0$).
...
__Frage:__ Unterscheiden sich die Gruppen in der Veränderung der Blutzuckerwerte?

```{r echo = FALSE, fig.width=4, fig.height=4,out.width="4.5cm", fig.align='center',warning=FALSE,message=FALSE}
path <- "../../../5_some_examples/data_examples/anova/Blutzucker/"
d.blz <- read.table(here("5_some_examples/data_examples/anova/Blutzucker/blutzucker.dat"),header=T)
...
\normalsize
Question: Why is `mutate()` needed first?

## Multiple comparisons, multiple tests

To remember:

* The $F$-Test is used to check whether \alert{any two group means} differ. 
* Using pairwise tests is not a very good idea (see slide 10), because this leads to a \alert{multiple testing problem}:

 \colorbox{lightgray}{\begin{minipage}{14cm}
When many tests are carried out, the probability to find a "significant" result {\bf by chance} increases.
\end{minipage}}

For instance, for four groups there are $4\cdot 3/2= 6$ pairwise combinations that could be tested. 

$\rightarrow$ The probability to find \emph{at least one result by pure chance under $H_0$} is much higher than the 5\% error level!!!

## Post-hoc tests

__Still:__ If the test $\beta_2=\ldots= \beta_g=0$ is rejected, a researcher is then often interested 
...
__Idea:__ If a total of $m$ tests are carried out, simply divide the type-I error level $\alpha_0$ (often 5\%) such that

$$\alpha = \alpha_0 / m \ .$$

\alert{\bf Tukey HSD approach}

__Idea:__ Take into account the distribution of \emph{ranges} (max-min) and design a new test.

\alert{\bf Fisher's LSD approach}

__Idea:__ Adjust the idea of a two-sample test, but use a larger variance (namely the pooled variance of all groups).
...
& 1 & 2 & 3\\
\hline
2 & `r table.tukey["2-1","diff"]` &  & \\
3 & `r table.tukey["3-1","diff"]` & `r table.tukey["3-2","diff"]`\\
4 & `r table.tukey["4-1","diff"]` & `r round(table.tukey["4-2","diff"],1)` & `r format(table.tukey["4-3","diff"],1,1,1)`\\
\end{tabular}
...
& 1 & 2 & 3\\
\hline
2 & `r round(table.tukey["2-1","p adj"],2)` &  & \\
3 & `r format(table.tukey["3-1","p adj"],2,2,2)` & `r round(table.tukey["3-2","p adj"],2)`\\
4 & `r round(table.tukey["4-1","p adj"],2)` & `r round(table.tukey["4-2","p adj"],2)` & 
`r round(table.tukey["4-3","p adj"],3)`\\ 
\end{tabular}
...
& 1 & 2 & 3\\
\hline
2 & `r round(table.bonf[1,1],2)` &  & \\
3 & `r round(table.bonf[2,1],2)`  &  `r round(table.bonf[2,2],2)`  \\
4 & `r round(table.bonf[3,1],2)`  & `r format(table.bonf[3,2],2,2,2)`   & `r round(table.bonf[3,3],3)` \\
\end{tabular}
...
& 1 & 2 & 3\\
\hline
2 & `r round(table.fisher[1,1],2)` &  & \\
3 & `r round(table.fisher[2,1],2)`  &  `r round(table.fisher[2,2],3)`  \\
4 & `r round(table.fisher[3,1],2)`   & `r format(table.fisher[3,2],2,2,2)`   & `r round(table.fisher[3,3],4)` \\
\end{tabular}
\end{columns}
...
In the "Blutzucker" example, this could be, for instance:
\begin{center}{\bf "Is diet 1 different from diets 2 to 4?"}
\end{center}
...
Back to the Hybrid Mais example. R orders the levels alphabetically and takes the first level as reference level.
...
Example \scriptsize(from Hand et al. 1994 / Hothorn/Everitt "A Handbook of Statistical Analyses Using R"): 

\normalsize
Experiment to study the weight gain of rats, depending on four diets. Protein amounts were either high or low, and the protein source was either beef or cereal. 10 rats for each diet were selected.

__Question:__ How does diet affect weight gain?

__Complication:__ This is a factorial design (gekreuzte Faktoren), because each combination of  protein source (beef/cereal) $x$ level (high/low) is present (2 $x$ 2 groups).
...
In the presence of a \alert{factorial design}, the idea is to add separate effects $\beta_i$ (here $i=1,2$) and $\gamma_j$ (here $j=1,2$) for the $i$th level of the first factor and the $j$th level of the second factor:
...
Note: We again need additional constraints. Here we always use the R default (\alert{"treatment contrasts"})

\begin{itemize}
\item $\beta_1=\gamma_1=0$.
\end{itemize}

\scriptsize Alternative:  $\sum_i\beta_i = \sum_i\gamma_i = 0$ (\alert{sum-to-zero contrast}).
...
\normalsize
Interpretation: There seems to be a difference between low and high amounts of protein, but the source (beef/cereal) seems less relevant. 

However: what if the additive model does not hold?

## 

A so-called \alert{interaction plot} helps to understand if the additive model is reasonable:
...
__Note:__ if the additive model $\beta_i + \gamma_j$ holds, the lines would be parallel.

However, these lines are \alert{not parallel}, indicating that \alert{there is an interaction} between amount and source!
...
\alert{This works in the same way as for categorical covariates in regression!} To see this, let us estimate the means from the model. From the above output, we have \scriptsize [because of using treatment contrasts]:
...
\ 
$\hat{(\beta\gamma)}_{cereal/high}= -18.8$ $\hat{(\beta\gamma)}_{beef/high}=\hat{(\beta\gamma)}_{beef/low}=\hat(\beta\gamma)_{cereal/low}=0$.
...
__Be careful:__ In the presence of interactions, the $p$-values of the main effects can no longer be interpreted as before!
...
And finally, the model diagnostics:  
...
In an experiment the influence of four levels of fertilizer (DUENGER) on the yield (ERTRAG) on 5 species (SORTE) of crops was investigated. For each DUENGER $\times$ ERTRAG combination, 3 repeats were taken.   
...
Btw, the summary table with coefficients looks horrible and the __$p$-values are not meaningful__! (why?)
...
\begin{itemize}
\item The $t$-test to compare the mean of \alert{two gropus} is a \alert{special case of ANOVA}.
\item Even more, the $F$-test is a special case of the $t$-test: $F_{1,n} = t^2_n$.
...
\item ANOVA is traditionally most used to analyze \alert{experimental data}.
\end{itemize}
