...
Eight groups of beetles were exposed to carbon disulphide (an insecticide) for 5h. For each beetle it was then reported if it was killed or not (1 or 0), but the data were reported {\bf aggregated}:\\[6mm]
...
Clarification of the difference between count data and binomial data:\\[6mm]

{\bf Count data:} theoretically no upper limit on number of times an ``event'' (e.g., number of birds observed in a forest plot), not possible to express as a proportion.\\[4mm]

{\bf Binomial data:} upper limit on number of times and ``event'' is observed (e.g., number of deaths cannot be greater than number of living individuals), possible to express as proportion.
}
...
\begin{itemize}
\item $\E(Y) = \pi =\P(y_i=1)$ {\tiny (useful to remember)}\\[2mm]
...
\colorbox{lightgray}{\begin{minipage}{10cm}
In order to prevent the expected value $\E(y_i)$ to attain unreasonable values, we thus formulate the \alert{logistic regression model} as
\begin{equation*}
...
\item Luckily, the \texttt{glm()} function in R can also handle binary data!\\[4mm]
...
Let's look at the results, for the moment on the coefficients:\\[2mm]
...
and standard errors and $p$-values. Very clearly, the dose influences the survival probability ($p<<0.001$), and $\hat\beta_1 >0$, thus, \alert{the larger the dose, the larger the mortality probability} (positive relation). \\[2mm]
...
\P(y_i=1)= \frac{\exp(\beta_0 + \beta_1 Dose_i)}{1 + \exp(\beta_0 + \beta_1 Dose_i)} \ . 
\end{equation}
...
\begin{equation*}
odds(y_i=1 \given Dose_i) = \frac{\P(y_i=1)}{\P(y_i=0)}= \exp(\beta_0 + \beta_1 Dose_i)\ . 
\end{equation*}
\end{minipage}}
}
...
{\bf Interpretation:} When the dose is increased by 1 unit, the odds to die increases by a factor of $\Sexpr{format(exp(summary(beetle.glm)$coef[2,1]),2,2,2)}$.\\[8mm]

Moreover, taking the $\log$ on the above equation shows that \alert{$\beta_1$ can be interpreted as a log odds ratio}:
\begin{equation*}
...
\begin{itemize}
\item In the beetle example, we were in a comfortable situation: For each level of the does, we had several beetles. For instance, 55 beetles at lowest does (49.06), of which 6 died (1) and 49 survived (0). This was \alert{binomial} data, an aggregated version of 55 trials with 0 or 1 outcome.\\[3mm]
...
{\bf Example}: Blood screeing {\tiny (see week 1; data from Hothorn \& Everitt 2014, chapter 7.3)}
}
...
{\bf  Specifically: } Is there an association between ESR level ESR$<20mm/hr$ and the concentrations of the plasma proteins Fibrinogen and Globulin?\\[6mm]
...
\frame[containsverbatim]{
It is a bit more illustrative to give a \alert{conditional density plot}: 
\vspace{-2mm}
...
For non-aggregated data, the \texttt{residual deviance} vs.\ \texttt{df} relation cannot be used to detect overdispersion!!
}
