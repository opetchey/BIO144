...
\item  \emph{The Essential Guide to Effect Sizes. Statistical Power, Meta-Analysis, and the Interpretation of Research Results} (2010, First Edition) Ellis. Ebook via \href{http://ezproxy.uzh.ch/login?url=http://dx.doi.org/10.1017/CBO9780511761676} {\beamergotobutton{UZH library}}.\\[3mm]
\item \emph{Regression - Modelle, Methoden und Anwendungen} by Fahrmeier, Kneib, Lang (for a deeper understanding) 
\end{itemize}
}
...
plot.1 <- ggplot(d.hg,aes(x=log(Hg_soil))) + geom_histogram(bins=30) + xlab("Hg soil")
plot.2 <- ggplot(d.hg,aes(x=log(Hg_urin))) + geom_histogram(bins=30) + xlab("Hg urine")
plot_grid(plot.1, plot.2,ncol=2)
@
\end{center}
}
