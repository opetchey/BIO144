...
plot.1 <- ggplot(d.hg,aes(x=amalgam,y=Hg_urin)) + geom_point() + 
  theme_bw() +
  xlab("Amalgam") +
  ylab("Hg urine")

plot.2 <- ggplot(d.hg,aes(x=age,y=Hg_urin)) + geom_point() + 
  theme_bw() +
  xlab("Age") +
  ylab("Hg urine")

plot.3 <- ggplot(d.hg,aes(x=fish,y=Hg_urin)) + geom_point() + 
  theme_bw() +
  xlab("Fish consumption") +
  ylab("Hg urine")

plot_grid(plot.1, plot.2, plot.3, labels = c('a', 'b','c'),ncol=3)
@ 
\end{center}
\vspace{5mm}
