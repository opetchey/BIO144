...
\date[]{Vorlesung 1: Einf\"uhrung und Ausblick\\ 23./24. Feburar 2017}
...
All wichtigen Details, wie Testatbedingung, Prf\"ufungsdaten etc. finden Sie auf der OpenEdX Kursseite:\\[2mm]

\url{https://openedx.mnf.uzh.ch/courses/course-v1:UZH+BIO144+FS2017/about}
\vspace{10mm}

Vorlesungszeiten jeweils :00 bis :45\\
\begin{center}$\rightarrow$ Letzte Lektion: {\bf 16:00 bis 16:45}.\end{center}
...
\item \emph{Getting Started with R, An introduction for biologists} (2017, {\bf Second Edition}) Beckerman, Childs \& Petchey (VERWENDEN SIE NICHT DIE 1. AUSGABE!). \\
{\scriptsize Ein UZH library Link zur 2. Ausgabe wird so bald wie m\"oglich zur Verf\"ugung gestellt.  }
...
\item \emph{Regression - Modelle, Methoden und Anwendungen} von Fahrmeier, Kneib, Lang\\[3mm]
...
\frame{\frametitle{Kurs-Fahrplan (12 Wochen Vorlesung + 2 Wochen Selbststudium)}
\vspace{-8mm}

\begin{multicols}{2}
{\bf Woche 1} Einf\"uhrung und Ausblick \\[1mm]
{\bf Woche 2} Einfache lineare Regression\\[1mm]
{\bf Woche 3} Residuenanalyse, Modellvalidierung \\[1mm]
{\bf Woche 4} Multiple lineare Regression \\[1mm]
{\bf Woche 5} ANOVA  \\[1mm]
{\bf Woche 6} ANCOVA, Matrix Algebra \\[1mm]
{\bf Woche 7} Modellwahl  \\[1mm]
{\bf Selbststudiums-Woche } \\[4mm]

{\bf Woche 8} Interpretation von Resultaten, Kausalit\"at \\[1mm]
{\bf Woche 9} Z\"ahldaten (Poisson Regression) \\[1mm]
{\bf Woche 10} Bin\"are Daten (logistische Regression)\\[1mm]
{\bf Woche 11} Messfehler, zuf\"allige Effekte \\[1mm]
{\bf Selbststudiums-Woche}  \\[1mm]
{\bf Woche 12} Ausgew\"ahlte Themen, Wiederholungen und Ausblick \\
\end{multicols}
~\\
...
\item Wir m\"ochten Ihnen eine herausforderne, spannende und freudvolle Lernerfahrung geben. {\bf Etwas, was man wirklich brauchen kann und darum Spass macht.}\\[6mm]
\end{itemize}

Unsere \"Uberzeugung: Fundierte Kenntnisse in Statistik machen Sie unabh\"angig! \\[2mm]
...
Erkenntnis, dass ohne Kenntnisse in statistischer Datenanalyse eigene Daten in Bachelor-, Master- oder Doktorarbeiten nicht ausgewertet werden k\"onnen. \\[6mm]

Beispiele:
\begin{itemize}
\item Medizin: Hat ein bestimmtes Medikament eine Wirkung? Welche Faktoren führen zu Krebs?
\item \"Okologie: Was für einen Lebensraum braucht ein Tier zum Leben? Was bevorzugt es?
...
\begin{itemize}
\item \alert{Auffinden} und \alert{Quantifizierung von Zusammenh\"angen} durch graphische Darstellung und Modellierung.\\[3mm]
\item Aus Daten g\"ultige \alert{Schlussfolgerungen ziehen}.\\[3mm]
\item Die \alert{Unsicherheit} der Schlussfolgerung \alert{quantifizieren}.
\end{itemize}
}
...
\emph{Forschungsfrage:} Welche Einflussfaktoren beeinflussen das Bewegungsverhalten von 2-6 j\"ahrigen Kindern?\\[4mm]
...
\frame{\frametitle{}
Einige einzelne {\bf Streudiagramme}:\\[2mm]
...
\frame[containsverbatim]{\frametitle{Beispiel 2: Quecksilber (Hg) im Wallis}
...
\begin{center}
\setkeys{Gin}{width=0.65\textwidth}
<<hg1,fig=T,height=4,width=4,echo=F>>=
...
plot((Hg_urin) ~ amalgam, data=d.hg, xlab="Amalgamfuellungen", ylab = "Hg Urin")
plot((Hg_urin) ~ age, data=d.hg, xlab="Alter", ylab = "")
plot((Hg_urin) ~ fish, data=d.hg, xlab="Meerfischkonsum/Monat", ylab = "")
@ 
\end{center}
\vspace{5mm}

Aus diesen Grafiken (Streudiagrammen) is es sehr schwer zu sagen, welche Faktoren die Quecksilberbelastung im Menschen genau beeinflussen.

}


\frame[containsverbatim]{\frametitle{}
Es ist immer n\"utzlich, die Verteilungen der Variablen im Modell anzuschauen. Zeichnen wir mal das {\bf Histogramm} der Quecksilberwerte:\\[4mm]

\begin{center}
\setkeys{Gin}{width=1\textwidth}
<<hghist,fig=T,height=4,width=7,echo=F>>=
par(mfrow=c(1,2))
...
In solchen F\"allen kann es helfen, die Variable zu \emph{logarithmieren}.
 ...
 \begin{center}
\setkeys{Gin}{width=1\textwidth}
<<hghist2,fig=T,height=4,width=7,echo=F>>=
par(mfrow=c(1,2))
#ggplot(d.hg,aes(log(Hg_soil))) + geom_histogram() +  theme_bw() + geom_histogram(log(Hg_urin))
...
plot(log(Hg_urin) ~ log(Hg_soil), data=d.hg, xlab="log(Hg Boden)",
     ylab = "log(Hg Urin)",pch=21,bg=as.numeric(mother)+2,xlim=c(-4.5,4.5),cex=1.2)

# ggplot(d.hg, aes(x = log(Hg_soil), y = log(Hg_urin),col=mother)) +
# xlim(-4.5,4.5) +
# geom_point(size = 2) +
# xlab("log(Hg Boden)") +
# ylab("log(Hg Urin)") +
...
F\"ur mehrere Gruppen braucht man die \alert{\emph{Varianzanalyse}} oder \alert{\emph{ANOVA}} (=ANalysis Of VAriance; siehe Kapitel 10.1 des Luchsinger-Skripts).\\[4mm]

Es wird sich herausstellen (Vorlesung 5), dass sich die Di\"aten tats\"achlich voneinander unterscheiden. \\[4mm]
...
\frame[containsverbatim]{\frametitle{}
Die Unterteilung ESR$<20mm/hr$ vs.\ ESR$\geq 20mm/hr$ f\"uhrt zu einer \myalert{bin\"aren} Response-Variablen.\\

Der Zusammenhang der einzelnen Plasmaprotein-Levels kann mit einer grafischen Darstellung, dem \myalert{\emph{conditional density plot}}, gut erfasst werden:
...
Ein Modell ist eine Ann\"aherung an die Realit\"at. Das {\bf Ziel der Statistik und Datenanalyse} ist es immer, dank Vereinfachungen der wahren Welt gewisse {\bf Zusammenh\"ange zu erkennen}.\\[6mm]
...
\item {\bf Daten graphisch darstellen}
\item Ein geeignetes \emph{Modell} ausw\"ahlen
\item Modellparameter und deren Unsicherheit sch\"atzen
\item {\bf Modellannahmen \"uberpr\"ufen}
\item Falls notwendig, Modell verbessern; zur\"uck zu Schritt 6
\item {\bf Resultate interpretieren} und mit Schritt 1 vergleichen
\item {\bf Resultate pr\"azise und verst\"andlich kommunizieren} (Publikation, Zeitungsbericht...)
\end{enumerate}
...
\begin{enumerate}[a)]
\item \myalert{Vorhersage (Extrapolation), Interpolation}. Beispiel K\"orperfett: verwende Ersatzumessungen, um K\"orperfett einer Person vorherzusagen.\\[3mm]
\item \myalert{Sch\"atzung von Parametern}. Beispiel: Effektgr\"osse eines neuen Medikamentes.\\[3mm]
\item \myalert{Verstehen; Bestimmung von Einflussgr\"ossen}. Beispiel Aktivit\"atsstudie bei Kindern: Es werden Faktoren gesucht, welche das Bewegungsverhalten der Kinder (positiv oder negativ) beeinflussen.\\[3mm]
...
%Barplots & \"Ahnlicher Anwendungsbereich wie Boxplots, aber nicht empfohlen.\\[1mm]
...
% par(mfrow=c(1,2))
% comp.dat <- read.csv("../../data_examples/GSWR_datasets/compensation.csv")
% 
...
% 
% mids <- barplot(mean.fruit,xlab="Treatment",ylab="Fruit Production",ylim=c(0,100),plot=F)
% 
% barplot(mean.fruit,xlab="Treatment",ylab="Fruit Production",ylim=c(0,100),main="")
% arrows(mids,mean.fruit - sd.fruit,mids,mean.fruit+sd.fruit,code=3,angle=90,length=0.1,add=T)
% text(mids,3,paste("n=",n.fruit),add=T)
% 
...
Dazu gibt es etlich R Pakete. Interaktive Darstellungen k\"onnen beispielsweise mit Shiny Apps generiert werden (siehe ``census app'').
%' <<>>=
%' library(shiny)
...
%\frame{\frametitle{Experimentelle vs Beobachtungsdaten}
%To do, ev mention only later in lectures 7/8
%}
