...
\date[]{Lecture 10: Modelling binary data  \\ 17./18. May 2018}
...
Note that I have also uploaded the continuation of the Stahel script, chapters 7-9 that cover GLM models. This is {\bf not} mandatory literature.
}
...
\begin{itemize}
\item The {\bf odds} (``Wettverh\"altnis''): For a probability $\pi$ the odds is $\pi/(1-\pi)$. For example, if the probability to win a game is 0.75, then the odds is given as 0.75/0.25 or 3:1.\\[4mm]
...
~\\
To understand what $\beta_1$ tells us, let's rearrange the equation. Some algebra leads to 
...
~\\[2mm]
\colorbox{lightgray}{\begin{minipage}{10cm}
It is then possible to calculate the \alert{odds} (``Chance'') with some algebra:
\begin{equation*}
...
\begin{itemize}
\item In the beetle example, we were in a comfortable situation: For each level of the does, we had several beetles. For instance, 49 beetles at lowest dose (49.06), of which 6 died (1) and 43 survived (0). This was \alert{binomial} data, an aggregated version of 55 trials with 0 or 1 outcome.\\[3mm]
...
{\bf  Specifically: } Are the concentrations of the plasma proteins Fibrinogen and Globulin (which are disease indicators) associated with the probability that ESR$>20mm/hr$?\\[6mm]
...
Plotting the response $y$ (indictor for ESR$>20$) against the covariats does not lead to very illustrative graphs:\\[4mm]

\begin{center}
\setkeys{Gin}{width=0.9\textwidth}
<<fig=T,echo=F,width=5,height=2.3>>=
require(cowplot)
theme_set(theme_cowplot(font_size=10))

p1 <- ggplot(plasma, aes(x = fibrinogen, y = y)) + theme_bw() +
geom_point() +
xlab("Fibrinogen") + ylab("ESR>20") 

p2 <- ggplot(plasma, aes(x = globulin, y = y)) + theme_bw() +
geom_point() +
xlab("Globulin") + ylab("ESR>20") 
...
\begin{center}
\setkeys{Gin}{width=0.6\textwidth}
<<fig=T,width=4,height=3.8,echo=F>>=
...
\begin{enumerate}
\item Is there an an effect of fibrinogen and/or globulin on the outcome (ESR$>20$)?\\[2mm]
