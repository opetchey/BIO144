...
subtitle: "Lecture 10: Modeling count data"
...
\textbf{How do the covariates influence the probability of a certain count outcome?}
...
```{r echo = FALSE}
print('missing DATA')
#soay <- read.table("../../../3_datasets/soay_mass_data.csv",header = T, sep = ',')
```
...
```{r echo = FALSE, fig.width=4, fig.height=4,out.width="6cm"}
...
```{r echo = FALSE, fig.width=6, fig.height=6,out.width="5.6cm"}
...
```{r echo = FALSE, fig.width=6, fig.height=6,out.width="6.5cm"}
# autoplot(r.soay)
```
...
\textbf{$\rightarrow$ Because nothing prevents $\eta_i$ from being negative!}\\[4mm]
...
```{r echo = FALSE, fig.width=5.5, fig.height=5.5,out.width="6.2cm"}
...
```{r echo = FALSE, message=FALSE, warning=FALSE, fig.width=4, fig.height=4,out.width="6cm"}
...
However, the \alert{variance is often larger than the mean} in reality, because there are factors that influence the response ("cause variability in it") that cannot be captured by the covariates!  
