...
\item Chapters 9.1 - 9.3 from \emph{The new statistics with R} (Hector book).
\end{itemize}
}
...
{\bf  Specifically: } Is there an association between ESR level ESR$<20mm/hr$ and the concentrations of the plasma proteins Fibrinogen and Globulin (which are disease indicators)?\\[6mm]
...
\end{equation*}
\\[4mm]
with $\E(y_i)=\P(y_i=1)=\pi_i$, or, equivalently: $y_i \sim \Bern(\pi_i)$.
}
...
\begin{center}$\Leftrightarrow \quad$ $\exp(\beta)$ is an odds ratio \\\end{center}
