...
\begin{itemize}
\item Repetition: Chapter 10 in Stahels book ``Statistische Datenanalyse'' from last semester.\\[4mm]
\item Chapters 9.1 - 9.3 from ``The new statistics with R'' (Hector book).
\end{itemize}
}
...
\item Counts cannot be expressed as a proportion.\\[4mm]
...
\begin{itemize}
\item Repeate the experiment with $\P(Y=1)=\pi$ a total number of $n$ times, calculate how often a success was observed ($k$ times).
...
\vspace{8mm}
{\small Example: In the beetle data  49 beetles were tested for the lowest dose, of which $k=6$ died.}
}
...
with standard errors and $p$-values. Very clearly, the dose influences the survival probability ($p<<0.001$), and $\hat\beta_1 >0$, thus, \alert{the larger the dose, the larger the mortality probability} (positive relation). \\[2mm]
