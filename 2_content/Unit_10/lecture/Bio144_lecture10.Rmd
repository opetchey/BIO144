...
\item Often, however, the response variable will be a categorical, or count variable.\\[2mm]
...
Ecologists were interested whether body mass of females influences their fitness, measured as their \alert{lifetime reproductive success} (the number of offspring over their lifespan).
...
There is still a clear upward trend in the scale-location plot, indicating that the \alert{variance increases} as the fitted values get larger.
...
with respective standard errors and $p$-values. In particular, $p<0.001$ for $\hat\beta_1$ indicates \alert{very strong evidence for a positive effect of female weight on reproductive success} (number of offspring).
...
\textbf{Note:} The deviance is essentially a difference of likelihoods. Think of it as the "Maximum-Likelihood Estimation" (MLE) \alert{equivalent to the Sums of Squares} in OLS regression.

## 

Here, the \alert{total deviance} is the so-called \alert{NULL} deviance: 85.081. It is analogue to the total variability of the data in linear regression.

Of this, 37.041 is \alert{explained by bodysize}.

The question is, whether this is much? This can be tested by a $\chi^2$ test (deviance follows a $\chi^2$- rather than an F-distribution:
...
A new drug was tested in a clinical trial (Giardiello et al. 1993, Piantodsi 1997), aiming to \alert{reduce the number of polyps} in the colon (Dickdarm). The data are publicly available from the Hothorn/Everitt book package:
\  
...
\textbf{Question:} Does the drug influence (reduce) the number of polyps?
...
\textbf{Remember:} The variance of the Poisson distribution increases with the mean, because \alert{\textbf{mean= variance}} (see slide 13). 

In Poisson regression it is assumed that, for each observation $i$,
$$y_i \sim Po (E(y_i)) \ .$$
  
  However, the \alert{variance is often larger than the mean} in reality, because there are factors that influence the response that cannot be captured by the explanatory variables.  
...
\end{itemize}
This estimates the variance parameter (denoted as \alert{dispersion parameter}) from the data.
...
Can it happen that the observations are \alert{less variable} than expected?
...
In that case, your $p$-values may be too large, that is, the results are \alert{overly conservative}.

The \texttt{quasipoisson} regression is a pragmatic solution in that scenario as well (a negative binomial regression is not!)
