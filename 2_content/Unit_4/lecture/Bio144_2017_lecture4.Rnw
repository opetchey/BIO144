...
par(mfrow=c(1,2))
plot(r.hg$fitted,r.hg$residuals,xlab="Fitted values", ylab="Residuals")
abline(h=0,lty=2)
...
\myalert{Note:} these four diagrams are plotted automatically by R when you use the \texttt{plot()} or the \texttt{autoplot()} function on an \texttt{lm} object, for example \texttt{autoplot(r.hg)}.
}
...
It is smometimes useful to enrich the TA-plot by adding a ``running mean'' or a ``smoothed mean'', which can give hints on the trend of the residuals. For the mercury example where $\log(Hg_{\text{urin}})$ is regressed on smoking, amalgam  and fish consumption for mothers only (slides 33-35 or week 3): 
\vspace{-10mm}
...
par(mfrow=c(1,3))
plot(1:length(residuals(r1.urin.mother)),residuals(r1.urin.mother),xlab="Observation number",ylab="residual")
abline(h=0,lty=2)
plot(d.hg.m$amalgam,residuals(r1.urin.mother),xlab="No. of amalgam fillings",ylab="residual")
abline(h=0,lty=2)
plot(d.hg.m$fish,residuals(r1.urin.mother),xlab="No. of fish meals/moth",ylab="residual")
abline(h=0,lty=2)
@
\end{center}
}
...
The scale-location plot is particularly suited to check the assumption of equal variances ({\bf homoscedasticity / Homoskedasdizit\"at}).\\[2mm]
