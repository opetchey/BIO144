...
where $\bm{x}^{(1)}$ is a continuous variable, and $\bm{x}^{(2)}$ is a binary grouping variable (thus taking values 0 and 1 to indicate the group).\\[4mm]
...
The idea is to plot the square root of the residuals $\sqrt{|R_i|}$ against the fitted values $\hat{y_i}$. There should be {\bf no trend} {\scriptsize (Continue with the Hg example)}:\\ 
\vspace{-3mm}
\begin{center}
\setkeys{Gin}{width=0.6\textwidth}
