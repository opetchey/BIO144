...
\date[]{Lecture 4: Multiple linear regression (finalize) / Residual analysis / Checking modeling assumptions\\ 15./16. March 2018}
...
\item The $F$-test is a \alert{generalization of the $t$-test}, which is used to test if $\beta_j = 0$ for one single variable $x^{(j)}$.
\end{itemize}
}
...
@
\end{center}
{\bf Important:} This model will be fitted in the BC videos of week 5.
}
...
An important observation is that children and mothers show different dependencies of age!\\[2mm]
...
This can be achieved by introducing a so-called \alert{interaction term} into the regression equation.\\[2mm]
...
What is the advantage of this? Why is this usually better than fitting two separate models, one for children and one for mothers?\\[5mm]
...
Remember that in linear regression the modeling assumption is that the residuals $e_i$ are independently normally distributed around zero, that is, $e_i \sim \N(0,\sigma_e^2)$. This implies four things:\\[6mm]
...
Complete overview of those used in this course:\\[2mm]
...
<<hgFigCov,fig=T,echo=F,width=7,height=2.3>>=
library(ggplot2)
library(gridExtra)
...
\begin{center}
\setkeys{Gin}{width=0.6\textwidth}
<<hgFigQQ,fig=T,echo=F,width=4,height=3.5>>=
library(ggfortify)
...
<<echo=F,fig=T,width=6,height=2.1>>=
set.seed(37489)
...
{\scriptsize (See Stahel chapter 4.5)}\\[2mm]
