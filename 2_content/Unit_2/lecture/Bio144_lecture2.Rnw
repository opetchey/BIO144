...
\date[]{Lecture 2: Simple linear regression\\ 1./2. March 2018}
...
Even simpler question: How good is BMI as a predictor for body fat?\\[8mm]
...
<<fig=T,width=3.5,height=3,echo=F>>=
library(ggplot2)
#plot(bodyfat ~ bmi,d.bodyfat,xlab="BMI (x)", ylab="body fat (y)")
ggplot(d.bodyfat,aes(x=bmi,y=bodyfat)) + geom_point() + theme_bw()
@
\end{center}
...
<<fig=T,width=4.5,height=4.5,echo=F>>=
...
\item another realization of the data (other 243 males) would lead to a slightly different picture.\\[4mm]
\end{itemize}
...
Remember: There are \myalert{three parameters} $\alpha$, $\beta$ and $\sigma_e^2$ to be estimated.\\[4mm]
...
Plotting the resulting line into the scatterplot is simple:\\[10mm]
...
library(biostatUZH)
@
<<plotbodyfat,fig=T,width=4,height=3>>=
...
The assumption we took here is that the errors $e_i \sim \N(0,\sigma_e^2)$. This implies {\bf four things}:\\[2mm]
\colorbox{lightgray}{\begin{minipage}{10cm}
\begin{enumerate}[a)]
...
<<fig=T,width=7,height=3.5,echo=F>>=
par(mfrow=c(1,2))
 # library(reshape2)
 # pars <- melt(pars)
...
\item the formulas to calculate the variances depend on the residual variance $\sigma_e^2$, the sample size $n$ and SSQ$^{(X)(\star)}$.
\end{itemize}
\end{minipage}}
...
Generally, $R^2$ indicates the proportion of variability of the response variable $\bm{y}$ that is {\bf explained by the ensemble of all covariates}.\\[4mm] 
...
\colorbox{lightgray}{\begin{minipage}{10cm}
%For \emph{each} $x_0$ one obtains a confidence interval for the expected value $\hat y_0 = \hat\alpha + \hat\beta x_0$. 
...
\frame{\frametitle{Tasks until next week}

The idea of the course is that as a preparation for next week's practical part you will do the following:\\[2mm]
