...
<<fig=T,width=3.5,height=4,echo=F>>=
library(ggplot2)
...
library(biostatUZH)
@
<<plotbodyfat,fig=T,width=4.5,height=4.5>>=
plot(bodyfat ~ bmi,d.bodyfat,xlab="BMI")
...
The {\bf larger} $R^2$ \\ 
$\Rightarrow$  the {\bf more} variability of $\bm{y}$ is captured by the covariates \\
...
\frame[containsverbatim]{
For task 2 above we can use the built-in ``distribution table'' of the t-distribution in R (two-sided $p$-value):\\[2mm]
