...
\begin{center}
<<fig1, eval=T,fig.width=5, fig.height=4,warning=FALSE,out.width="6.5cm">>=
library(ggplot2)
...
\item Test for several $\beta_1 = \beta_2 = ... = \beta_{k}=0$ simultaneously $\rightarrow$ $F$-test.\\
...
\colorbox{lightgray}{\begin{minipage}{10cm}
\begin{enumerate}[a)]
\item The expected value of each residual $\epsilon_i$ is 0: $\E(\epsilon_i)=0$.\\[2mm]
...
A dependency would be reflected by some kind of \myalert{trend}. \\[8mm]
...
In the leverage plot, (standardized) residuals $\tilde{R_i}$ are plotted against the leverage $H_{ii}$ (continuing with the Hg example):\\[2mm]
