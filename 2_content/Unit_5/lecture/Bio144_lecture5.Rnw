...
\date[]{Lecture 5: ANOVA \\ 22./23. March 2018}
...
\begin{itemize}
\item Generally: Are the means of two or more groups different?\\[2mm]
\item Example: Are different plant breeds different in important aspects (e.g., yields / Ertrag)?\\[2mm]
...
The clou is: Model \eqref{eq:aov} is identical to the regression model with a factor covariate, see slides 35/36 from week 3. 
\end{minipage}}
~\\[4mm]
...
\vspace{-4mm}
\begin{eqnarray*}
\bm{H_0} &:& \mu_1=\mu_2=\ldots = \mu_g  \quad \text{or, equivalently} \quad  \beta_1=\beta_2 = \ldots = \beta_g   \\[2mm]
\bm{H_1} &:& \text{At least two groups are different}
\end{eqnarray*}
\end{minipage}}

~\\[4mm]
Remember from slides 45/46 of week 3 (or Stahel script p.33): We have already used the $F$-test for categorical variables (see $F$-test for the earthworms, slide 47 of week 3). This was equivalent to testing if all $\beta$s that belong to a categorical variable are =0 at the same time. \\[2mm]

$\rightarrow$ equivalent to test if the (categorical!) covariate is needed in the model.\\[2mm] 
...
To derive the ingredients of the $F$-test, we look at the decomposition of variance (Remember this idea from week 3, slide 23):
\vspace{-4mm}
...
$\rightarrow$ The larger $F$, the less likely is the data seen under $H_0$. \\[4mm]
...
@
~\\[-2mm]
As always, before looking at the results, let's do some model checking. This is identical to all we did so far, because we are \alert{still working with linear models!}
\vspace{-2mm}
\begin{center}
\setkeys{Gin}{width=0.5\textwidth}
...
{\bf Always} when we needed to do an $F$-test and when categorical covariates were involved, the \texttt{anova()} table was required:\\[3mm]
...
You can see that the value of $F=\Sexpr{format(summary(r.mais)$fstatistic[1],nsmall=2,digits=2)}$ is $F$-distributed with 3 and 16 degrees of freedom, and the $p$-value of the test ``$\beta_2=\beta_3=\beta_4$ ?'' is $<0.0001$.\\[4mm]
...
\frame[containsverbatim]{
What happens if you apply \texttt{summary()} to the \texttt{lm()} object?\\[4mm]
...
The table contains the estimates of the intercept \Sexpr{format(summary(r.mais)$coef[1,1],2,2,2)} ($\mu$ in ANOVA notation, $\beta_0$ in regression notation), and estimates for $\beta_2$, $\beta_3$, $\beta_4$ (while the reference was set to $\beta_1=0$).\\[2mm]
{\scriptsize {\bf Hint:} Check also the $F$-test on the last line...}
}
...
\begin{center}{\bf ``Is diet 1 different from diets 2-4?''}\\[10mm]
\end{center}
...
This can be changed manually:\\[2mm]
...
~\\
Start by looking at means and standard deviations in the groups, as well as at a graphical description of the means: \\[2mm]
<<echo=F>>=
library(HSAUR3)
d.weightgain <- mutate(weightgain,type = relevel(type,ref="Low"))
@
<<echo=T>>=
d.weightgain %>% group_by(source,type) %>% summarise(meanW = mean(weightgain),sdW = sd(weightgain))
@
~\\[2mm]
\begin{multicols}{2}
\setkeys{Gin}{width=0.5\textwidth}
...
<<>>=
r.weight <- lm(weightgain ~ source + type, d.weightgain)
anova(r.weight)
@
~\\[4mm]
...
sumWeight <- summarise(group_by(d.weightgain,source,type),meanWeight = mean(weightgain),sdW = sd(weightgain))
...
However, these lines are \myalert{not parallel}, indicating that \myalert{there is an interaction} between type and source!\\[4mm]

In words: The type (low/how) has a different influence for the Beef and Cereal diets.
...
Let's include an interaction term in the rats example:\\[4mm]

<<>>=
r.weight2 <- lm(weightgain ~ source * type,d.weightgain)
anova(r.weight2)
@
~\\[4mm]
...
It is then required that separate ``stratified'' analyses are carried out. For example for "Beef" and "Cereal" protein sources:\\[4mm]

<<>>=
anova(lm(weightgain ~ type,subset(d.weightgain,source=="Beef")))
anova(lm(weightgain ~ type,subset(d.weightgain,source=="Cereal")))
@
...
In an experiment the influence of four levels of fertilizer (DUENGER) on the yield (ERTRAG) on 5 species (SORTE) of crops was investigated. For each DUENGER $\times$ ERTRAG level, 3 repeats were taken. \\[2mm]
...
~\\[2mm]
What is  the problem??\\[4mm] 
$\rightarrow$ Look at the TA and the scale-location plots. \\[2mm]
$\rightarrow$ Ideas? \\[2mm]
