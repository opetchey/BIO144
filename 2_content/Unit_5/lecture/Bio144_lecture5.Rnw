...
\date[]{Lecture 5: Multiple linear regression (finalize) / Residual analysis / Checking modeling assumptions\\ 15./16. March 2018}
...
\colorbox{lightgray}{\begin{minipage}{10cm}
\begin{enumerate}[a)]
\item The expected value of each residual $e_i$ is 0: $\E(e_i)=0$.\\[2mm]
...
\begin{center}
\setkeys{Gin}{width=0.5\textwidth}
<<slPlot,fig=T,echo=F,width=3.5,height=3.0>>=
