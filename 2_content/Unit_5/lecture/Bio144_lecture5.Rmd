---
title: "Lecture 5: Multiple linear regression (finalize) / Residual analysis / Checking modeling assumptions"
...
* \alert{Binary} and \alert{categorical} explanatory variables: The idea is to introduce \alert{dummy variables} such that
...
* The categorical explanatory variables of last week were used to allow for \alert{group-specific intercepts} (see earthworm example).
...
Question: Given multiple regression explanatory variables ${x}^{(1)}, {x}^{(2)},...$. Could I simply fit separate simple models for each variable, that is
...
```{r eval=T,warning=FALSE,out.width="4cm",echo=FALSE, fig.align='center'}
library(ggfortify)
...
```{r eval=T,warning=FALSE,out.width="4cm", fig.align='center', echo=FALSE}
...
2. If not, ask whether the model has been misspecified. Do reasonable transformations of the response and/or explanatory variables eliminate the outlier? Do the residuals have a distribution with a long tail (which makes it more likely that 3. extreme observations occur)?
...
5. Check in inclusion/exclusion changes the qualitative conclusion, and by how much it changes the quantiative conclusion.
