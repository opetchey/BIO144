...
\date[]{Lecture 12 (part 2): Measurement error in regression models \\ 27. May 2019}
...
$\rightarrow$  Basically, a fundamental assumption always is that the {\bf distributional assumptions are fulfilled}.
}
...
$\rightarrow$ the error variance is simply absorbed in the residual variance $\sigma^2_\epsilon$. 
\vspace{-2mm}
\begin{center}
