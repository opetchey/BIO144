...
## Unit 12
class_RTs_filtered %>%
  ggplot(aes(x = Pref_Reaction_time_ave,
...
## Unit 12
class_RTs_filtered %>%
  pivot_longer(cols = c("Pref_Reaction_time_ave", "Pref_Reaction_time_1"),
               names_to = "Variable",
               values_to = "Values") %>%
  ggplot(aes(x = Values, y = Nonpref_Reaction_time_ave, col = Variable)) +
  geom_point() +
  geom_smooth(method = "lm")
...
m1 <- lm(Nonpref_Reaction_time_ave ~ Pref_Reaction_time_ave,
         data = class_RTs_filtered, x = TRUE)
summary(m1)


dd2 <- select(class_RTs_filtered,
...
## Unit 12
class_RTs_filtered %>%
  pivot_longer(cols = c("Pref_Reaction_time", "Pref_Reaction_time_1"),
               names_to = "Variable",
               values_to = "Values") %>%
  ggplot(aes(x = Values, y=Nonpref_Reaction_time_ave, col = Variable)) +
  geom_point() +
