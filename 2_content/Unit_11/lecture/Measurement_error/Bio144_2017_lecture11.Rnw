...
\begin{itemize}
\item ME in in covariates ($x$) and in the response ($y$) of regression models.
...
\begin{itemize}
\item {\bf Linear regrssion including ANOVA:} 
...
\item Basically, a fundamental assumption is that the distributional assumptions are fulfilled.\\[4mm]
\end{itemize}
}


\frame{\frametitle{Another fundamental assumption (but often neglected)}
...
\item Generalized linear and non-linear regressions (e.g.\ logistic an Poisson).\\[2mm]
\end{itemize}
...
\frame{
	...
	\begin{eqnarray*}
	w_i &=& x_i + u_i \nonumber\\
	u_i &\sim& \mathcal{N}(0,\sigma^2_{u})  \  \label{CErrorModel} \ , \nonumber
	\end{eqnarray*}
...
<<echo=FALSE,fig=TRUE,height=2.5,width=2.3>>=
tx<-seq(-4,4,0.01)
par(mfrow=c(1,1))
...
\frame{\frametitle{Illustration of the problem}
	...
	\begin{eqnarray*}
	y_i= 1 \cdot x_i + \epsilon_i, \quad \epsilon_i \sim \N(0,\sigma^2_{\epsilon}) \ .
	\end{eqnarray*}
...
<<echo=FALSE, fig=TRUE, width=4.3, height=3>>=
library(ggplot2)
set.seed(84522)
col1 <- "red"
col2 <- "blue"
...
\item ME \alert{masks imporant features} of the data, making graphical model inspection difficult.\\[6mm]
\end{enumerate}

\begin{center}
\setkeys{Gin}{width=.5\textwidth}
<<echo=FALSE, fig=TRUE, width=4.5, height=3>>=
library(ggplot2)
set.seed(84522)
col1 <- "red"
col2 <- "blue"
...
\frame{\frametitle{Error in the outcome of regression models (ev delete?)}
...
\end{center}
\vspace{-8mm}
$\rightarrow$ Error in the response seems to be less of a problem. However, this might not be true for other regression types (logistic, Poisson) or error structures.
}
...
{\bf Example}: If classical error $w_i = x_i + u_i$ with $u_i \sim \N(0,\sigma_u^2)$ is present, knowledge of the {\bf error variance} $\sigma_u^2$ is needed.\\[4mm]
\item In \alert{simple cases}, formulas for the bias exist. \\[2mm]
{\bf Example}: In linear regression the biased slope parameter $\beta_x^\star$ relates to the true value $\beta_x$ as 
$$\beta_x^\star = \left(\frac{\sigma_x^2}{\sigma_x^2 + \sigma_u^2}\right) \beta_x \ .$$
$\rightarrow$ knowing $\sigma_u^2$ and $\sigma_x^2$, the correct slope can be calculated.\\[2mm]
\item In most cases, such simple relations don't exist. Specific error modelling methods are then needed!
\end{itemize}
...
<<fig=T,width=4.5,height=3.8,echo=F>>=
set.seed(212356)
sigmax <- 1
sigmau <- 0.25
...
{\bf Problem:} The BMI was self-reported and thus suffers from measurement error! Not $x_i$ are observed, but only 
