...
\frame{\frametitle{Another fundamental assumption that is often neglected}
...
\end{center}
\vspace{-8mm}
$\rightarrow$ Error in the response seems to be less of a problem. However, this is \emph{not} true for other regression types (logistic, Poisson) or error structures.
}
...
Given the simple linear regression equation $y_i = \beta_0 + \beta_x x_i + e_i$ with $w_i = x_i + u_i$.
Assume that $w_i$ instead of $x_i$ is used in the regression:
$$y_i = \beta^\star_0 + \beta^\star_x w_i + e_i \ .$$
~\\
...
r.simex <- simex(r.lm,SIMEXvariable="BMI",measurement.error=sqrt(4),
                 lambda=seq(0,2.5,0.1),B=10,fitting.method="quadratic")
...
{\bf Note:} The \texttt{sex} variable has \emph{not} been mismeasured, but it is was alsozs affected by the error in BMI.\\[2mm]
