...
\item Also look at \alert{relative importances} of explanatory variables.\\[2mm]
\item {\bf NEVER use $p$-values for model selection.} \\[2mm]
\end{enumerate}

## Suggestion 1: Graded interpretation of $p$-values

** Potential addition for 2022**: https://www.cell.com/trends/ecology-evolution/fulltext/S0169-5347(21)00284-6
...
\textbf{Remember:} $R^2$ indicates the proportion of variance explained by \textbf{all} explanatory variables in a model
\begin{equation*}
...
Look at a regression model including explanatory variable ${x}$ and response ${y}$. If the coefficient $\beta_x$ is "significant", there are several possible reasons for this:
...
\item There is another explanatory variable ${z}$ that both influences ${x}$ and ${y}$\\
