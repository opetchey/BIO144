...
subtitle: "Lecture 9: Interpretation, causality, cautionary notes"
...
#d.hg <- read.table("../../../5_some_examples/data_examples/Hg/Hg_urin.csv",header=T, sep=",")
...
## Test yourself: Klicker-Exercise
...
## What is the problem with the $p$-value? II
  ...
  \item It is much better to have a more \alert{gradual interpretation of the $p$-value} (see slide 18).\\[8mm]
  \end{itemize}
...
\vspace{8mm}
\begin{eqnarray} 
y_i = \beta_0 + \beta_{1}  x^{(1)}_i  +   + \epsilon_i   & \quad &  R^2 = `r print('format(summary(r.lm_fish)$r.squared,nsmall=2,digits=1)')`\\
y_i = \beta_0 + \beta_{2}  x^{(2)}_i  +   + \epsilon_i   & \quad &  R^2 = `r print('format(summary(r.lm_amalgam)$r.squared,nsmall=2,digits=1)')` \\
y_i = \beta_0 + \beta_{1}   x^{(1)}_i  + \beta_{2}  x^{(2)}_i    + \epsilon_i  & \quad & R^2 = 
`r print('format(summary(r.lm_fish_amalgam)$r.squared,2,2,2)')`
\end{eqnarray}
-->
...
```{r echo = FALSE, eval = FALSE}
<!-- \begin{itemize} -->
<!-- \item {\bf Model 1:} Relative importance of \texttt{fish}: `r format(lmg.hg[8]*100,2,2,2)`\% (see slide \ref{sl:relimp}).\\[2mm] -->
<!-- \item {\bf Model 2:} Relative importance of \texttt{fish}: $`r format(lmg.hg.2[8]*100,2,2,2)`$\% .\\[8mm]   -->
<!-- \end{itemize} -->
```
