...
\frame{\frametitle{Collinearity of covariates}
...
\frame{\frametitle{}
In statistics, the expression ``collinearity'' is also used when such a collinearity relationship is approximately true. For example, when two variables $\bm{x^{(1)}}$ and $\bm{x^{(2)}}$ have a high correlation.\\[4mm]

{\bf What is the problem with collinearity?}

I like to do simple (and extreme) examples to understand the point. Given two identical covariates $\bm{x^{(1)}}=\bm{x^{(2)}}$, an extreme form of ``high correlation'', and the regression model
\begin{equation*}
y_i = \beta_0 + \beta_1 x_i^{(1)} + \beta_2 x_i^{(2)} + e_i \ ,
\end{equation*}

the slope coefficients $\beta_1$ and $\beta_2$ \alert{cannot be uniquely determined} (there are many solutions to the equation)! \\[2mm]

When the variables are collinear, this problem is less severe, but the $\beta$ coefficients can be estimated \alert{less precisely} (with higher standard error, thus larger $p$-value).
}
...
\item See also Stahel 5.4(i) for  ``recipe''.
\end{itemize}}
...
$\rightarrow$ The AIC for the optimal model is $\Sexpr{format(AIC(r.AIC),1,1,1)}$, compared to the full model with an AIC of $\Sexpr{format(AIC(r.bodyfat),1,1,1)}$. This is not a very dramatic improvement.
}
