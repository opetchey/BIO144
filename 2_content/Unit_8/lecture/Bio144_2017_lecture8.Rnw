...
\begin{itemize}
\item Literature/articles from the self-study week (todo: say which ones here)
\end{itemize}
...
\frame{
Add thought as given in Stahel 3.3 f, g and especially h (other variable Z that influences both X and Y.)
}
...
\item The $p$-value is not ``bad'', but it has to be used properly \\
$\rightarrow$ 3 suggestions or alternatives (gradual interpretation on $p$-values, effect sizes and CIs, relative importances).\\[2mm]
