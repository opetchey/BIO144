...
* __Select your covariates according to \alert{a priori} knowledge.__
...
* Covariates are sometimes \alert{collinear}, which leads to more uncertainty in the estimation of the respective regression parameters, and thus to larger $p$-values.
...
* The $p$-value for mercury in soil, $MISSING VALUE$, is rather high: $MISSIING VALUE$.

* __Question:__ What is the answer to the main research question?
...
A model checking step (always needed, but we did it alreayd in lecture 5):

```{r fig.width=7, fig.height=4,out.width="8cm"}
library(ggfortify)
...
Given a set of covariates ${x^{(1)}}, {x^{(2)}}, {x^{(3)}}, ...,{x^{(p)}}$. If it is possible for one of the covariates to be written as a \alert{linear combination of the others}
\begin{equation*}
x_i^{(j)} = \sum_{k\neq j} c_k x_i^{(k)} \quad \text{for all} \quad  i=1,...,n
\end{equation*}

the set of covariates is said to be \alert{collinear}.  


__Examples:__ 

* Three vectors in a 2D-plane are always collinear.
* A covariate can be written as a linear combination of two others: $x^{(j)} = c_1\cdot x^{(1)} + c_2 \cdot x^{(2)}$, then the three variables are collinear.
...
A simple (and extreme) example to understand the point: Assume two covariates are identical ${x^{(1)}}={x^{(2)}}$. In the regression model
\begin{equation*}
...
The \alert{variance inflation factor} (VIF) is a measure for collinearity. It is defined for each covariate ${x^{(j)}}$ as 
\begin{equation*}
VIF_j = \frac{1}{1-R_j^2} \qquad
\end{equation*}
 where $R_j^2$ is the $R^2$ of the regression of ${{x^{(j)}}}$ against all other covariates. (Note: if $R_j^2$ is large, this means large collinearity and thus a large VIF).  
...
* Read the assigned literature to prepare for week 10.
