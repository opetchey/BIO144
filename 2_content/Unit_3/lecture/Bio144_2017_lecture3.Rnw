...
<<QQ1,fig=T,echo=T,width=4,height=4>>=
...
\colorbox{lightgray}{\begin{minipage}{10cm}
\begin{eqnarray*}
y_i &=& \beta_0 + \beta_1 x_i^{(1)} + \beta_2 x_i^{(2)} + \ldots + \beta_m x_i^{(m)} + e_i  \\[2mm]
...
Again, a 95\% CI for $\beta_j$ can be calculated with $[\hat\beta - c \cdot \sigma^{(\beta)} ; \hat\beta + c \cdot \sigma^{(\beta)}]$, where $c$ is the 97.5\% quantile of the $t$-distribution with $n-p$ degrees of freedom  (compare to slides 38-40 of last week). With R:\\[4mm]
...
{\bf Example:} Abdomen (Bauchumfang) seems to be a relevant predictor and it is obvious that abdomen and BMI are correlated:
\vspace{-5mm}
\begin{center}
\setkeys{Gin}{width=0.4\textwidth}
<<fig=T,width=4,height=4,echo=F>>=
plot(abdomen ~ bmi,d.bodyfat)
@
\end{center}
\vspace{-1mm}

\colorbox{lightgray}{\begin{minipage}{10cm}
This problem of \alert{collinearity} is at the heart of many confusions of regression analysis, and we will talk about such issues later in the course (lectures 7 and 8).
\end{minipage}}

\vspace{2mm}
Please see also IC: practical 3 (week 4) for an example and more thoughts.
...
Penalization for adding more independent variables that do not fit the model!\\[2mm]
...
\frame[containsverbatim]{\frametitle{}
\begin{enumerate}
\item Are the modelling assumptions met?
...
We first glimpse at a check of the modelling assumptions:\\[2mm]
\setkeys{Gin}{width=0.8\textwidth}
<<Hgcheck,fig=T,width=6,height=3.5,echo=F>>=
par(mfrow=c(1,2))
...
\vspace{6mm}
However:  this model is \alert{not identifiable}. I could add a constant to $\beta_1, \beta_2, ...\beta_k$ and subtract it from $\beta_0$, and the model would fit as good to the data.\\[2mm]
...
Data set of $n=143$ worms with three species (Lumbricus, Nicodrilus, Octolasion), weight, stomachic circumference (Magenumfang). 
...
Therefore, \alert{log-transform} the response (weight), and it looks much better:\\[2mm]
...
% \begin{equation*}
% \log(\text{Gewicht}_i) = \beta_0 + \beta_1 \cdot \text{Magenumfang}_i + \beta_2 \cdot \text{Gattung}N_i + \beta_3 \cdot \text{Gattung}Oc_i + e_i. 
% \end{equation*}
This is imple in R:\\ 
{\scriptsize (But make sure that Gattung is stored as a factor in R (check by \texttt{str(d.wurm)})!)} \\
...
~\\
Before doing anything else, check the modelling assumptions:\\[-4mm]
\begin{center}
\setkeys{Gin}{width=0.7\textwidth}
<<fig=T,echo=F,widht=8,height=3.5>>=
par(mfrow=c(1,2))
plot(r.lm$fitted,r.lm$residuals,xlab="fitted",ylab="residuals")
abline(h=0,lty=2)
...
{Answer:} We need 4 parameters: $\beta_0$, $\beta_1$, $\beta_2$, $\beta_3$, thus $143-4=139$.\\[2mm]
...
\item No coefficient is needed for Gattung=Lumbricus, it is automatically set to 0.
\end{itemize}
}


\frame[containsverbatim]{\frametitle{Interpreting the results II}
{\bf Main question:} Is there a realation between stomach size and body mass? \\[4mm]

{\bf Results:} \texttt{MAGENUMF} is a positive coefficient with $p<0.0001$, thus very strong evidence that the relation exists. Increasing \texttt{MAGENUMF} by 1 unit increases $\log_{10}$(\texttt{GEWICHT}) by +0.31.\\[4mm]
...
{\bf Problem:} The $p$-values of the worm species are not very meaningful. They belong to tests that compare the actual level with the reference level. However, the question is whether the species variable has an effect in total.\\[6mm]
...
~\\[2mm]
{\bf Note:} Here, the $F$-value for Gattung is distributed as $F_{2,139}$ under the Null-Hypothesis.
~\\[4mm]
Here we have $p=\Sexpr{format(anova(r.lm)[2,5],2,4,2)}$, thus a clear difference in the regression models for the three species (``Gattung is relevant'').
}
