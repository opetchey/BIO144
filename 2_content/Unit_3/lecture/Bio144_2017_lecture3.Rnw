...
@
~\\[2mm]
There is some weak ($p=0.12$) indication that smokers have a slightly increased Hg concentration in their body.\\[4mm]
...
Formulate a model 
\begin{equation*}
\log(\text{Gewicht}_i) = \beta_0 + \beta_1 \cdot \text{Magenumfang}_i + \beta_2 \cdot \text{Gattung}_i + e_i. 
\end{equation*}
...
\frame[containsverbatim]{\frametitle{Interpreting the results}


{\bf Question:} Is the ``Gattung'' covariate relevant in the model, that is, do the models differ for the three species? \\[6mm]
...
{\bf Solution:} When a factor covariate with $k$ levels is in the model, it occupies $k-1$ parameters. Therefore, the $t$-test needs to be replaced by the $F$-test.
