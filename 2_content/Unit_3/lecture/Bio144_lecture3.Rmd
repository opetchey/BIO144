...
* Is the relationship different from what we would expect by change?
...
## What assumptions to we make?

The main assumption in linear regression is that the residuals follow a $N(0,\sigma^2)$ distribution.

We make this assumption because it is often well enough met, and give great mathematical tractibility.
...
- (d) we assumed a linear relationship (related to (c) above).
...
## (d) Linearity (related to independence)
...
```{r eval = TRUE, message=FALSE, fig.width=5, fig.height=4,warning=FALSE,echo=F,out.width="7cm", fig.align='center'}
...
It is sometimes useful to enrich the TA-plot by adding a "running mean" or a "smoothed mean", which can give hints on the trend of the residuals. However, generally we recommend to \alert{not} add a smoothing line, because it may bias our view on the plot.
...
\hspace{1cm}$\rightarrow$ \href{http://students.brown.edu/seeing-theory/regression-analysis/index.html}{Click here} to do it manually!
