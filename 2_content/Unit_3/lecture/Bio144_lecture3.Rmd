---
title: "Lecture 3: Simple linear regression"
...
* Here, $\beta_0$ is the \alert{axis intercept} and $\beta_1$ the \alert{slope} of the line. $\beta_1$ is also denoted as the regression coefficient of $X$.
...
\colorbox{lightgray}{\begin{minipage}{14cm}
The parameters $\beta_0$ and $\beta_1$ are estimated such that the sum of \alert{squared vertical distances} (sum of squared residuals)
...
\
Typically, the question is whether the slope $\beta_1$ might be 0 or not, that is: "Is there an effect of the covariate $x$ or not?"  
\
...
T=\frac{\hat\beta_1 - C}{se^{(\beta_1)}}\ . %\quad \text{with} \quad se^{(\beta_1)}=\sqrt{\hat\sigma_e^2/SSQ^{(X)}}  \ .
\end{equation}
\end{minipage}}
\end{center}

Again: typically, $C=0$, so the formula simplifies to $T=\frac{\hat\beta_1}{se^{(\beta_1)}}$.
...
Note: The last column contains the \textbf{$p$-value} of the test $\beta_1=0$.
...
Conclusion: there is __very strong evidence__ that the BMI is associated with bodyfat, because $p$ is extremely small (thus it is very unlikely that such a slope $\hat\beta_1$ would be seen if there was no effect of BMI on body fat).
...
Note: For the confidence range, only the uncertainty in the estimates $\hat\beta_0$ and $\hat\beta_1$ matters.
