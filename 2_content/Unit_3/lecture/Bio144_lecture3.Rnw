...
\date[]{Lecture 3: Multiple linear regression \\ 8./9. March 2018}
...
$$\sum_{i=1}^n r_i^2$$
with 
$$r_i = y_i - (\beta_0 + \beta_1 x_i^{(1)} + \beta_2 x_i^{(2)} + \ldots + \beta_m x_i^{(m)}) $$ 
...
\begin{enumerate}
\item Does the {\bf ensemble} of all covariates have an effect on the response?\\[4mm]
\item If yes, which influence variables are relevant predictors of bodyfat? \\[4mm]
\item Which proportion of response variability is explained by the model?
\end{enumerate}
...
\frame[containsverbatim]{\frametitle{Question 1: Do the covariates have an effect on the response?}
\vspace{-2mm}
...
\item Do the covariates improve the model (compared to a model without covariates)?
\item Which variable(s) influence(s) the response?
\item Which proportion of variability is explained?
...
First check the modelling assumptions:\\[2mm]
\setkeys{Gin}{width=0.8\textwidth}
