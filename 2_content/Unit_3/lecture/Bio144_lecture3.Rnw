...
\item Estimate the parameters $\alpha$, $\beta$ and $\sigma_e^2$ by  \href{http://students.brown.edu/seeing-theory/regression-analysis/index.html#section1}{\textcolor{blue}{least squares}}.\\[2mm]
...
\textcolor{gray}{Optional:You may want to watch the youtube video given here for more explanation
\href{https://www.youtube.com/watch?v=-KXy4i8awOg}{\bf here}.}
}
...
We have so far modeled bodyfat in dependence of bmi, that is: $(body fat)_i = \alpha + \beta \cdot bmi_i + e_i$.\\[2mm]
...
\begin{center}
\setkeys{Gin}{width=0.9\textwidth}
<<modelChecks,fig=T,width=6,height=3,echo=F>>=
library(ggfortify)
...
Again, a 95\% CI for $\beta_j$ can be calculated with R: \\[4mm] 
...
~\\[2mm]
{\scriptsize (The CI is again $[\hat\beta - c \cdot \sigma^{(\beta)} ; \hat\beta + c \cdot \sigma^{(\beta)}]$, where $c$ is the 97.5\% quantile of the $t$-distribution with $n-p$ degrees of freedom; compare to slides 37-39 of last week).}
}
...
{\scriptsize $^\star$ What does that mean? I could add a constant to $\beta_1, \beta_2, ...\beta_k$ and subtract it from $\beta_0$, and the model would fit as good to the data.} 
