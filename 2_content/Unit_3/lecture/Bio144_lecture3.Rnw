...
You may want to watch the youtube video given here for more explanation:
\url{https://www.youtube.com/watch?v=-KXy4i8awOg}
}
...
\item If yes, which covariates are relevant predictors of bodyfat? \\[4mm]
...
Again, a 95\% CI for $\beta_j$ can be calculated with $[\hat\beta - c \cdot \sigma^{(\beta)} ; \hat\beta + c \cdot \sigma^{(\beta)}]$, where $c$ is the 97.5\% quantile of the $t$-distribution with $n-p$ degrees of freedom  (compare to slides 37-39 of last week). With R:\\[4mm]
...
Again look at the data first:

\setkeys{Gin}{width=0.7\textwidth}
<<pairs,width=6,length=6,fig=T,echo=F>>=
pairs(d.cath)
@
...
\item Are both covariates needed?
...
\begin{center}
\setkeys{Gin}{width=0.8\textwidth}
<<Hgcheck,fig=T,width=5,height=2.5,echo=F>>=
library(ggfortify)
...
Some covariates indicate a {\bf category}, for instance the species of an animal or a plant. This type of covariatc is called a {\bf factor}. The trick: convert a factor with $k$ levels (for instance 3 species) into $k$ dummy variables $x_i^{(j)}$ with 
\colorbox{lightgray}{\begin{minipage}{10cm}
\begin{equation*}
x_i^{(j)} = \left\{ 
\begin{array} {ll}
...
The function \texttt{anova()} in R does the $F$-test for categorical variables. \\[6mm]
