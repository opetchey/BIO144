...
\begin{multicols}{2}
1. (L1) Introduction and outlook \\[1mm]
2. (L2) Simple linear regression\\[1mm]
3. (L3) Residual analysis, model validation\\[1mm]
4. (L4) Multiple linear regression \\[1mm]
5. (L5) ANOVA  \\[1mm]
6. (L6) ANCOVA Matrix Algebra \\[1mm]
7. (L7) Model selection  \\[1mm]
8. Self-study week \\[4mm]
9. (L8) Interpretation of results, causality \\[1mm]
10. (L9) Count data (Poisson regression) \\[1mm]
11. (L10) Binary Data (logistic regression)\\[1mm]
12. (L11) Measurement error, random effects \\[1mm]
13. Self-study week \\[1mm]
14. (L12) Selected topics, repetition and outlook \\
\end{multicols}
~\\
...
plot((Hg_urin) ~ amalgam, data=d.hg, xlab="amalgam", ylab = "Hg Urin")
plot((Hg_urin) ~ age, data=d.hg, xlab="age", ylab = "")
plot((Hg_urin) ~ fish, data=d.hg, xlab="fish consumption", ylab = "")
@ 
\end{center}
\vspace{5mm}
...
ggplot(d.blz,aes(x=factor(DIAET),y=BLUTZUCK)) + geom_boxplot() + geom_point(size=4,colour='lightgrey') + theme_bw() + xlab("Diet") + ylab("Blood glucose")
@
\end{center}
