...
\date[]{Week 1: Introduction and Outlook\\ 23./24. February 2017}
...
\frame{\frametitle{Schedule (12 weeks)}
\vspace{-8mm}
...
7. Model choice  \\[1mm]
8. Interpretation of the results \\[1mm]
9. Binary Data (logistic regression)\\[1mm]
10. Count data (Poisson regression) \\[1mm]
11. Measurement error, random effects \\[1mm]
12. Selected topics, repetition and outlook \\
\end{multicols}
~\\
...
{\bf Question:}  Which factors allow an precise estimation (prediction) of body fat? \\[4mm]

Study with 241 male participants. Measured variable were, among others, body fat (in \%), age, weight, body size, BMI, neck thickness and abdominal girth.\\[4mm]
...
plot((Hg_urin) ~ amalgam_quant, data=d.hg, xlab="amalgam", ylab = "Hg Urin")
plot((Hg_urin) ~ age, data=d.hg, xlab="age", ylab = "")
plot((Hg_urin) ~ Meerfisch_quant, data=d.hg, xlab="fish consumption", ylab = "")
@ 
\end{center}
\vspace{5mm}
...
\frame[containsverbatim]{\frametitle{Data example 3: Diet and blood glucose}
\vspace{-3mm}
