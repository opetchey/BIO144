...
\frame{\frametitle{What is the problem with the $p$-value? II}
  ...
  \item It is much better to have a more \alert{gradual interpretation of the $p$-value} (see slide \ref{sl:pInt}).
  \end{itemize}
}
...
\frame{\frametitle{Example: Breakfast eating and obesity}
...
\item {\bf Biological gradient:} Greater exposure should generally lead to greater incidence of the effect.
\item {\bf Plausibility:} A plausible mechanism is helpful.
\item {\bf Coherence:} Coherence between findings in the lab and in the field / population increases the likelihood of an effect.
\item {\bf Analogy:} Similar factors have a similar effect.
...
\begin{tabular}{lll}
 ...
 {\bf Interpretation} & Clear,  & Difficult, \\
  & ``proofs'' causal relationship & especiallly w.r.t.\ causality
 \end{tabular}
}
...
\item Statistical significance vs biological relevance: Ask for the \alert{effect size} and reflect what it means, instead of only reporting $p$-values. \\[2mm]
\item The $p$-value is not ``bad'', but it has to be used properly $\rightarrow$ 3 suggestions for model interpretation (gradual $p$-values, effect sizes and CIs, relative importances).\\[2mm]
