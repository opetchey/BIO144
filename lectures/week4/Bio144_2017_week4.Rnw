...
par(mfrow=c(2,2),mar=c(4,4,2,0.5))
r1.urin.mother <- lm(log10(Hg_urin) ~  smoking + sqrt(amalgam) + sqrt(fish),data=d.hg.m)
plot(r1.urin.mother)
@
\end{center}
}
...
\item If not, ask whether the model has been misspecified. Do reasonable transformations of the response or the covariables eliminate the outlier? \\[2mm]
...
\frame{\frametitle{The outlier in the Hg study}\label{sl:outliersHg}
Mention here that individual 106 forgot to report that she has 5 amalgam fillings!
}
% \frame{References:
