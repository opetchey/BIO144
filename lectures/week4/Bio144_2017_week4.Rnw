...
$\Rightarrow$ \myalert{To find influential observations}
\end{itemize}
}
...
par(mfrow=c(1,2))
plot(d.hg$amalgam,residuals(r1.urin.mother),xlab="No. of amalgam fillings",ylab="residual")
abline(h=0,lty=2)
plot(d.hg$fish,residuals(r1.urin.mother),xlab="No. of fish meals/moth",ylab="residual")
abline(h=0,lty=2)
@
\end{center}
}
...
The idea is to plot the squared residuals $\sqrt{|R_i|}$ against the fitted values $\hat{y_i}$:\\[2mm]
\begin{center}
\setkeys{Gin}{width=0.6\textwidth}
