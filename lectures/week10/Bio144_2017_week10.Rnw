...
\date[]{Week 10: Modelling count data \\ 4./5. May 2017}
...
@
\end{center}
$\rightarrow$ The diagnostic plots show that the linear regression assumptions are badly violated!
}
...
autoplot(r2.soay,smooth.colour = NA)
@
\end{center}
$\rightarrow$ This looks better. However...
}

\frame{\frametitle{What is the problem?}
Obviously, the \alert{normal distribution} for count data is \alert{violated}. Why?\\[2mm]
...
\item The normal distribution is symmetrical, counts are often not!
\end{itemize}
}
...
\item $\E(Y) = \Var(Y) = \lambda$. In words: Mean = variance = $\lambda$.\\[2mm]
...
\begin{itemize}
\item Finally, we need a reasonable \alert{probability model for the response variable}. \\[2mm
]
\item Remember: we always used the normality assumption $y_i \sim \N(\eta_i, \sigma_e^2)$ in linear regression.\\[2mm]

\item Given that $y_i$ are counts, a Poisson model seems reasonable:  
$$y_i \sim \Po (\E(y_i)) \ .$$
~\\
In words: $y_i$ is a realization of a Poisson random variable distributed as $\Po(\lambda_i)$, where $\lambda_i=\E(y_i)$.\\[2mm]
...
\eta_i = \beta_0 + \beta_1 x_i^{(1)} + \beta_2 x_i^{(2)} + \ldots + \beta_p x_i^{(p)} \ .
\end{equation*}
In the soay shee example, it is $\eta_i = \beta_0 + \beta_1\cdot \text{bodySize}_i$ for sheep $i$.\\[2mm]
...
Uff... that was hard! But we have all the tools for fitting a Poisson GLM now. A statistician would now say: \\[2mm]
\begin{center}
...
\frame{
\begin{itemize}
\item Model diagnostics seem ok (similar to when we used a quadratic term in the linear model, see slide \ref{sl:quadratic}).\\[2mm]

\item Do you find it strange that the same diagnostic plot can be used as in linear regression?\\[2mm]
...
with respective standard deviations and $p$-values. In particular, $p<0.001$ for $\hat\beta_1$ indicates \alert{very strong evidence for a positive (because $\hat\beta_1 >0$!) effect  of female weight on reproductive success} (number of offspring)!\\[4mm]
...
Thus a 95\% CI can be approximated by the usual $\hat\beta\pm 2\cdot \sigma_\beta$ idea.
}
...
$$
\log(\E(y_i)) =\beta_0 + \beta_1 \cdot \text{bodySize} 
$$

~\\
Thus the $\exp(\cdot)$ transformation leads to 
$$ \quad \E(y_i) = \exp(\beta_0 + \beta_1 \cdot \text{bodySize})$$.\\[2mm]

Our estimates $\hat\beta_0$ and $\hat\beta_1$ can be plugged into this equation!\\[2mm]

For example, a 5~kg female is \alert{expected} to produce 
$\exp(\Sexpr{format(summary(soay.glm)$coef[1,1],3,3,3)} + \Sexpr{format(summary(soay.glm)$coef[2,1],3,3,3)}\cdot 5) = \Sexpr{format(exp(summary(soay.glm)$coef[1,1] + summary(soay.glm)$coef[2,1]*5),2,2,2)}$ lambs, while a 7~kg female would already be expected to produce 
$\exp(\Sexpr{format(summary(soay.glm)$coef[1,1],3,3,3)} + \Sexpr{format(summary(soay.glm)$coef[2,1],3,3,3)}\cdot 7) = \Sexpr{format(exp(summary(soay.glm)$coef[1,1] + summary(soay.glm)$coef[2,1]*7),2,2,2)}$ lambs.\\[2mm]
...
Here, it is useful again, but it gives us an \alert{Analysis of Deviance} table:\\[4mm]
...
Look at the analysis done in next three slides, and answer the following questions:\\[2mm]
...
\item The $p$-values for the coefficients are now larger! In particular, there is now only weak evidence ($p=0.063$) for an effect of age!\\[2mm]
...
You can {\bf detect it} by chekcing if \; \texttt{Residual deviance}$<$ \texttt{df}.\\[4mm]
