...
plant_gr <- read_csv("https://raw.githubusercontent.com/opetchey/BIO144/master/3_datasets/plant.growth.rate.csv")
...
m1 <- lm(plant.growth.rate ~ soil.moisture.content, data=plant_gr)
...
qplot(x = soil.moisture.content, y = plant.growth.rate, data=plant_gr) +
  ...
  xlab("Soil moisture content (ml per 10 grams)")
  theme_bw()
